...
class CollectionService:
    ...
    async def collect_beatport_tracks_raw(
        ...
        """Collect all raw data from Beatport and store it."""
        log.info(
        ...
        async with httpx.AsyncClient() as http_client:
            ...
            async with AsyncSessionLocal() as session:
                ...
                async for tracks_page in bp_client.get_tracks(
                    ...
                    await external_data_repo.bulk_upsert(bulk_data)
    ...
    async def process_unprocessed_beatport_tracks(
        ...
        async with AsyncSessionLocal() as session:
            external_data_repo = ExternalDataRepository(session)
            total_to_process = (
                await external_data_repo.count_unprocessed_beatport_tracks()
            )
